...
interface EventWithCount extends Event {
  // Optional: server-provided initialData has no count yet — the capacity
  // display below already guards on `registration_count !== undefined`.
...
export default function EventDetailClient({
  ...
  // Seed from server-fetched row so content is in the HTML on first paint.
  // The effect below still refetches to pick up the registration count.
  const [event, setEvent] = useState<EventWithCount | null>(initialData ?? null);
  ...
  useEffect(() => {
    ...
    async function loadEvent() {
      ...
      if (result) {
        ...
        // network error) shouldn't blank out an event we know exists.
        setNotFound(true);
      }
      setLoading(false);
    }
  ...
  return (
    <div className="min-h-screen px-4 py-8 md:px-6 lg:px-8 max-w-2xl mx-auto">
      ...
      <div className="mt-8 rounded-xl border border-border bg-surface p-6 md:p-8">
        <CommunityThread eventId={event.id} defaultTopic="general" title="Event Discussion" />
      </div>
    </div>
  );
}
